...
- TensorBoard 验证图：`val_images/<dataset>/sample_NN` 显示 GT/预测对比，`val_images/<dataset>/probability_heatmap_NN` 显示全分辨率伪彩概率图，`val_images/<dataset>/probability_overlay_NN` 显示概率图与原图的叠加；三者均受 `--val-image-interval` 和 `--val-image-count` 控制。
